...
BddNode
BddNode::restrict(const BddNode& g) const
{
   assert(g != BddNode::_zero);
   if(g == BddNode::_one) return *this;
   if(isTerminal()) return *this;
   unsigned groot = g.getLevel();
   BddNode a = _BddMgr->getSupport(groot);
   // cout << "a" << endl << a << endl;
   BddNode f_na = getRightCofactor(groot);
   // cout << "f_na" << endl << f_na << endl;
   BddNode f_a = getLeftCofactor(groot);
   // cout << "f_a" << endl << f_a << endl;
   // cout << "g" << endl << g << endl;
   BddNode g_na = g.getRightCofactor(groot);
   // cout << "g_na" << endl << g_na << endl;
   BddNode g_a = g.getLeftCofactor(groot);
   // cout << "g_a" << endl << g_a << endl;
   if(g_na == BddNode::_zero){
      // cout << "g_na == 0" << endl;
      return f_a.restrict(g_a);
   }
   if(g_a == BddNode::_zero){
      // cout << "g_a == 0" << endl;
      return f_na.restrict(g_na);
   }
   if(f_a == f_na){
      // cout << "f_a == f_na" << endl;
      return restrict(g_a | g_na);
   }
   // cout << "else" << endl;
   return (~a & f_na.restrict(g_na)) | (a & f_a.restrict(g_a));
}
